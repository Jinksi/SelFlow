...
class SelFlowModel(object):
    def __init__(
        ...
        restore_model="./models/KITTI/no_census_no_occlusion",
        dataset_config={},
        self_supervision_config={},
    ):
    ...
    def test(
        ...
        iterator = dataset.create_one_shot_iterator(
            dataset.data_list, num_parallel_calls=self.num_input_threads
        )
        ...
        new_h = tf.where(
            tf.equal(tf.mod(h, 64), 0), h, (tf.to_int32(tf.floor(h / 64) + 1)) * 64
        )
        new_w = tf.where(
            tf.equal(tf.mod(w, 64), 0), w, (tf.to_int32(tf.floor(w / 64) + 1)) * 64
        )

        batch_img0 = tf.image.resize_images(
            batch_img0, [new_h, new_w], method=1, align_corners=True
        )
        batch_img1 = tf.image.resize_images(
            batch_img1, [new_h, new_w], method=1, align_corners=True
        )
        batch_img2 = tf.image.resize_images(
            batch_img2, [new_h, new_w], method=1, align_corners=True
        )

        flow_fw, flow_bw = pyramid_processing(
            batch_img0,
            batch_img1,
            batch_img2,
            train=False,
        ...
        for i in range(dataset.data_num):
            ...
            if keep_only_fw:
                ...
                imageio.imwrite(
                    f"{save_dir}/fw/{save_name_list[i]}_flow_fw.png",
                    np_flow_fw_color[0],
                )
                imageio.imwrite(
                    f"{save_dir}/bw/{save_name_list[i]}_flow_bw.png",
                    np_flow_bw_color[0],
                )
                if save_flo_files:
                    ...
                    write_flo(
                        f"{save_dir}/flo_fw/{save_name_list[i]}_flow_fw.flo",
                        np_flow_fw[0],
                    )
                    write_flo(
                        f"{save_dir}/flo_bw/{save_name_list[i]}_flow_bw.flo",
                        np_flow_fw[0],
                    )
